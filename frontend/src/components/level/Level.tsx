...
const Level: React.FC<LevelPageTypes> = ({
  tasksList,
  currentLevel,
  isLoader,
  currentTask,
  ...
  const sendAnswer = (answer: string) =>
    dispatch(setAnswerThunk(answer, currentTask.answer, currentLevel, userID));
          ...
          <Card className={modalStyles.cardImageBackground}>
            <CardContent>
              <img
                ...
                src={`${SERVER_URLS.FILES_URL}/${currentTask.src}`}
                alt=""
              />
            </CardContent>
          </Card>
              ...
              {!isFinished ? (
                <div className={styles.frlevel__inner}>
                  ...
                  <div className={styles.frlevel__right}>
                    {currentTask?.otherSrc != null ? (
                      <img
                        src={`${SERVER_URLS.FILES_URL}/${currentTask.otherSrc}`}
                      />
                    ) : (
                      ""
                    )}
                    {currentTask.others.length == 0 ? (
                      <>
                        <TextField
                          ...
                          fullWidth
                          onChange={(event: any) =>
                            sendInputData(event.target.value)
                          }
                        />
                        <FRbutton onClick={() => sendAnswer(answer)}>
                          Проверить
                        </FRbutton>
                      </>
                    ) : (
